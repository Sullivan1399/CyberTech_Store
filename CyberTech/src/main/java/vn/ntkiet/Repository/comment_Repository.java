...
public interface comment_Repository extends JpaRepository<Comments,Integer>
{
    ...
    @Query("SELECT new vn.ntkiet.DTO.response.product_Rating(AVG(c.rate) ,COUNT(*)) FROM Comments c WHERE c.product.ID= :productID")
    ...
    @Query("SELECT new vn.ntkiet.DTO.response.comment_Product(c.comment,c.user.name,c.datetime,c.rate)  FROM Comments c WHERE c.product.ID= :productID")
